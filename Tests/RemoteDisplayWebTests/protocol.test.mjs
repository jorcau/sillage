...
import {
  ...
  VIEWS,
  THEMES,
} from "../../Sources/RemoteDisplay/Web/renderer.js";

test("rejects malformed, nonfinite, oversized, and unknown protocol frames", () => {
  const frame = silentPacket();
  assert.equal(validPacket(frame), true);
  for (const patch of [
    { version: 2 },
    { sequence: -1 },
    { levels: [null, null] },
    ...
    { phase: Array(257).fill([0, 0]) },
    { spectrum: [NaN] },
    { waveform: [[0, 0, 0]] },
    { correlation: 2 },
    { presentation: { view: "unknown", theme: "mint", source: "demo" } },
  ]) {
...
test("VU reference and stops match the Mac renderer", () => {
  assert.equal(vuFraction(-90), 0);
  assert.equal(vuFraction(-15), 1);
  assert.equal(vuFraction(0), 1);
...
test("English and French cover all controls, views and themes", () => {
  const en = JSON.parse(
    fs.readFileSync(
      new URL(
        "../../Sources/RemoteDisplay/Web/locales/en.json",
        import.meta.url,
      ),
    ),
  );
  const fr = JSON.parse(
    fs.readFileSync(
      new URL(
        "../../Sources/RemoteDisplay/Web/locales/fr.json",
        import.meta.url,
      ),
    ),
  );
  assert.deepEqual(Object.keys(en).sort(), Object.keys(fr).sort());
  for (const key of [...VIEWS, ...Object.keys(THEMES)]) {
    assert.ok(en[key]);
    assert.ok(fr[key]);
  }
  const html = fs.readFileSync(
    new URL("../../Sources/RemoteDisplay/Web/index.html", import.meta.url),
    "utf8",
  );
